...
//    String[] headerName = new String[]{"Id", "Username", "Email", "Phone", ""};
    ...
    public DocumentMngtController(DocumentMngtView view) {
        ...
        initTable(DocumentMngService.getInstance().listDocuments(null));
        doAction();
    }
...
//                        DocumentDTO dto = DocumentMngService.getInstance().getDocumentById(userId);
//                        onUpdate(dto);
...
//                                            String userId = (String) item.getItemProperty("id").getValue();
//                                            ResultDTO res = DocumentMngService.getInstance().removeDocument(userId);
                        ...
                        new ConfirmDialog.Listener() {
                            ...
                            public void onClose(ConfirmDialog dialog) {
                                if (dialog.isConfirmed()) {
                                    // Confirmed to continue
                                    DocumentDTO documentDTO = (DocumentDTO) obj;
                                    ResultDTO res = DocumentMngService.getInstance().removeDocument(String.valueOf(documentDTO.getId()));
...
//        }
    ...
    private void onSearch() {
        List<DocumentDTO> lstUser = DocumentMngService.getInstance().listDocuments(view.getTxtUserName().getValue());
...
//
//        try {
//            List<Users> lstUser = UserMngService.getInstance().listDocuments(view.getTxtUserName().getValue());
                            ...
                            new ConfirmDialog.Listener() {
                                ...
                                public void onClose(ConfirmDialog dialog) {
                                    if (dialog.isConfirmed()) {
                                        ...
                                        if (isInsert) {
                                            res = DocumentMngService.getInstance().addDocument(dto);
                                            ComponentUtils.showNotification(BundleUtils.getString("common.button.add") + " " + res.getKey() + " " + res.getMessage());
                                        } else {
                                            res = DocumentMngService.getInstance().updateDocument(dto);
