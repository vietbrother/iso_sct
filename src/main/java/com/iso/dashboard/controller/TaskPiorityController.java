...
import com.iso.dashboard.view.CataTaskPriorityMngtView;
 ...
 * @author VIET_BROTHER
 */
public class TaskPiorityController {

    CataTaskPriorityMngtView view;

    CustomGridEditRow pagedTable;
    String prefix = "taskPriorityMgnt.list";//tien to trong file language
    String headerKey = "header.taskPriorityMgnt";//lay trong file cas
    ...
    public TaskPiorityController(CataTaskPriorityMngtView view) {
        this.view = view;
        this.pagedTable = view.getPagedTable();
        initTable(TaskPriorityMgntService.getInstance().getTaskPrioritys());
        doAction();
    }

    public void initTable(List<CTaskPriority> takGroups) {
        ...
        pagedTable.genGrid(container, prefix, headerColumn, null, new HandlerButtonActionGrid() {
            ...
            public void actionEdit(Object obj) {
                CTaskPriority taskGroupItem = (CTaskPriority) obj;
                //luu thong tin cap nhat
                String name = taskGroupItem.getTaskPiorityName();
                String color = taskGroupItem.getColor();
                Boolean status = taskGroupItem.getStatus();
                int id = taskGroupItem.getTaskPiorityId();
                String desc = taskGroupItem.getDescription();
                CTaskPriority taskGroup = new CTaskPriority();
                taskGroup.setTaskPiorityName(name);
                taskGroup.setColor(color);
                taskGroup.setDescription(desc);
                taskGroup.setStatus(status);
                if (id != 0) {
                    taskGroup.setTaskPiorityId(id);
                    ResultDTO res = TaskPriorityMgntService.getInstance().updateTaskPriority(taskGroup);
                    ComponentUtils.showNotification(BundleUtils.getString("common.button.update") + " " + res.getKey() + " " + res.getMessage());
                } else {
                    ResultDTO res = TaskPriorityMgntService.getInstance().addTaskPriority(taskGroup);
            ...
            public void actionDelete(Object obj) {
                CTaskPriority taskGroupItem = (CTaskPriority) obj;
                String id = taskGroupItem != null ? taskGroupItem.getTaskPiorityId().toString() : null;
                if (id != null) {
                    ConfirmDialog d = ConfirmDialog.show(UI.getCurrent(),
                        ...
                        if (dialog.isConfirmed()) {
                            ResultDTO res = TaskPriorityMgntService.getInstance().removeTaskPriority(id);
                            ComponentUtils.showNotification("Delete id : " + id + " " + res.getKey() + " " + res.getMessage());
                            reloadData(TaskPriorityMgntService.getInstance().getTaskPrioritys());
                        }
                    });
        ...
        pagedTable.getColumn("taskPiorityName").setEditorField(nameEditor);
...
//        pagedTable.getColumn("status").getEditorField().setCaption("");
        ...
        pagedTable.getFilter().setTextFilter("taskPiorityName", true, false, BundleUtils.getString("taskPriorityMgnt.list.taskPriorityName"));
    }

    public void reloadData(List<CTaskPriority> taskGroups) {
        pagedTable.setContainerDataSource(pagedTable.createWrapContainer(createContainer(taskGroups)));
    }

    public BeanItemContainer createContainer(List<CTaskPriority> taskGroups) {
        BeanItemContainer<CTaskPriority> container = new BeanItemContainer<CTaskPriority>(
                CTaskPriority.class);
    ...
    private void doAction() {
        view.getBtnAdd().addClickListener((Button.ClickEvent event) -> {
            List<CTaskPriority> taskGroups = TaskPriorityMgntService.getInstance().getTaskPrioritys();
            taskGroups.add(new CTaskPriority());
